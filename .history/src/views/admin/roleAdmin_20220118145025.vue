  ...
  <div class="main">
        ...
        >
          ...
          <el-table-column fixed="left" label="操作" width="250">
            <template #default>
              ...
              <el-button type="text" :icon="Filter" size="small"
                >分配权限</el-button
              >
            </template>
          </el-table-column>
    ...
    <div class="tree">
      <div class="operate">
        ...
        <el-button type="primary" size="small" :icon="Check" style="margin-left: 8px" @click="distributeFunc"> 保存 </el-button>
      </div>
...
import { Check, Delete, Edit, Filter } from "@element-plus/icons-vue";
...
const  distributeFunc = () => {
  ...
  let setCheckedKeys = proxy.$refs.tree.getCheckedKeys();
  api.common.saveAssignFunctions({roleId:currentRow.id,}).then((data:any) => {})
}
let currentRow = {}
const tableRowClick = (row:any) => {
  currentRow = row
  api.common.treeRoleFunctions({
    node: 'root',
    roleId:row.id
  }).then((data:any) => {
    if(data.success) {
