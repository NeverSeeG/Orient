<template>
  <el-scrollbar style="height: 100%">
    <div class="main">
      ...
      <div>
        <el-table highlight-current-row v-for="(table,index) in tableData" :data="table.tableData" :key="index" border fit style="width: 100%">
          <el-table-column label="" v-for="(column,index) in table.columns" :prop="column.prop" :label="column.label" ></el-table-column>
        </el-table>
      </div>
    </div>
  </el-scrollbar>
...
const query = () => {
  ...
  equipIds.forEach(async (equipId: { VALUE: string }) => {
    ...
    const results = await findParasDataByMongo(params);
    console.log('280-----results',results);
    if (results.length) {
