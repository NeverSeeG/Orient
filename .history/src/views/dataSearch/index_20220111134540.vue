...
const exportData = () => {
  ...
  equips.forEach((equip: { NAME: string; VALUE: string }) => {
    ...
    if (paramsMap.has(equip.VALUE + "_ID")&&paramsMap.get(equip.VALUE + "_ID")!==undefined) {
      paramsIds.push((paramsMap.get(equip.VALUE + "_ID")||[]).join(","));
    }
     if (paramsMap.has(equip.VALUE + "_NAME")&&paramsMap.get(equip.VALUE + "_NAME")!==undefined) {
      paramsIds.push((paramsMap.get(equip.VALUE + "_NAME")||[]).join(","));
    }
  });
