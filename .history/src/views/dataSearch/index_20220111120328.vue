...
const exportData = () => {
  ...
  let  paramsMap  = new Map<String, Array<ParamsCombo>>()
  paraIds.forEach( (paraId: {
      GAS_EQUIPMENT_560_ID: String;
      ID: String;
      NAME_4129: String;
    }) => {
      let paramsCombo = new ParamsCombo(paraId.ID, paraId.NAME_4129);
      if (paramsMap.get((paraId.GAS_EQUIPMENT_560_ID)+'_ID')?.length) {
        paramsMap.get(paraId.GAS_EQUIPMENT_560_ID)?.push(paramsCombo);
      } else {
        paramsMap.set((paraId.GAS_EQUIPMENT_560_ID)+'_ID', [paramsCombo]);
      }
       if (paramsMap.get((paraId.GAS_EQUIPMENT_560_ID)+'_NAME')?.length) {
        paramsMap.get(paraId.GAS_EQUIPMENT_560_ID)?.push(paramsCombo);
      } else {
        paramsMap.set((paraId.GAS_EQUIPMENT_560_ID)+'_NAME', [paramsCombo]);
      }
  });
